...
impl AccountsStorageImpl {
    ...
    pub async fn new(storage: StorageDartWrapper) -> anyhow::Result<AccountsStorageImpl> {
        Ok(Self {
            inner_storage: AccountsStorageBox::create(storage.get_storage().get_storage()).await?,
        })
    }
    ...
    pub async fn get_entries(&self) -> anyhow::Result<String> {
        self.inner_storage.get_entries().await
    }
    ...
    pub async fn add_account(&self, account: String) -> anyhow::Result<String> {
        self.inner_storage.add_account(account).await
    }
    ...
    pub async fn add_accounts(&self, accounts: String) -> anyhow::Result<String> {
        self.inner_storage.add_accounts(accounts).await
    }
    ...
    pub async fn add_token_wallets(
        &self,
        account_address: String,
        network_group: String,
    ...
    /// root_token_contracts - list of addresses of tokens in blockchain.
    /// Return true or throw error.
    pub async fn remove_token_wallets(
        &self,
        account_address: String,
        network_group: String,
        root_token_contracts: Vec<String>,
    ...
    pub async fn remove_account(&self, account_address: String) -> anyhow::Result<Option<String>> {
        self.inner_storage.remove_account(account_address).await
    }
    ...
    pub async fn remove_accounts(&self, account_addresses: Vec<String>) -> anyhow::Result<String> {
        self.inner_storage.remove_accounts(account_addresses).await
    }

    /// Clear storage and remove all data.
    /// Returns true or throw error
    pub async fn clear(&self) -> anyhow::Result<bool> {
        self.inner_storage.clear().await
    }

    /// Reload storage and read all data again.
    /// Returns true or throw error.
    pub async fn reload(&self) -> anyhow::Result<bool> {
        self.inner_storage.reload().await
    }
