...
impl KeystoreDartWrapper {
    ...
    pub async fn new(
        ...
        let keystore =
            KeyStoreApiBox::create(keystore_builder, storage.get_storage().get_storage()).await?;
        Ok(Self {
    ...
    pub async fn get_entries(&self) -> anyhow::Result<String> {
        self.inner_keystore.get_entries().await
    }
    ...
    pub async fn add_key(&self, signer: KeySigner, input: String) -> anyhow::Result<String> {
        self.inner_keystore.add_key(signer, input).await
    }
    ...
    pub async fn add_keys(&self, signer: KeySigner, input: String) -> anyhow::Result<String> {
        self.inner_keystore.add_keys(signer, input).await
    }
    ...
    pub async fn update_key(&self, signer: KeySigner, input: String) -> anyhow::Result<String> {
        self.inner_keystore.update_key(signer, input).await
    }
    ...
    pub async fn export_seed(&self, signer: KeySigner, input: String) -> anyhow::Result<String> {
        self.inner_keystore.export_seed(signer, input).await
    }
    ...
    pub async fn get_public_keys(
        ...
        self.inner_keystore.get_public_keys(signer, input).await
    }
    ...
    pub async fn decrypt(
        ...
        self.inner_keystore.decrypt(signer, data, input).await
    }
    ...
    pub async fn remove_key(&self, public_key: String) -> anyhow::Result<Option<String>> {
        self.inner_keystore.remove_key(public_key).await
    }

    /// Remove list of public key from KeyStore and return json-encoded list of KeyStoreEntry's
    /// that were removed or throw error.
    pub async fn remove_keys(&self, public_keys: Vec<String>) -> anyhow::Result<String> {
        self.inner_keystore.remove_keys(public_keys).await
    }
    ...
    pub async fn is_password_cached(
        &self,
        public_key: String,
        duration: u64,
    ) -> anyhow::Result<bool> {
    ...
    pub async fn clear_keystore(&self) -> anyhow::Result<String> {
        self.inner_keystore.clear_keystore().await
    }

    /// Try to reload all stored data.
    pub async fn reload_keystore(&self) -> anyhow::Result<String> {
        self.inner_keystore.reload_keystore().await
    }
