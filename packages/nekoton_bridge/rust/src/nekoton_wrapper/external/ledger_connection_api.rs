...
impl LedgerConnectionDartWrapper {
    ...
    pub fn new(
        ...
        on_sign: impl Fn(u16, SignatureContext, Vec<u8>) -> DartFnFuture<Vec<u8>>
            + Send
            + Sync
            + 'static,
        on_sign_transaction: impl Fn(u16, u16, SignatureContext, Vec<u8>, String) -> DartFnFuture<Vec<u8>>
            + Send
            + Sync
            + 'static,
...
pub struct LedgerConnectionImpl {
    ...
    pub on_sign: Arc<dyn Fn(u16, SignatureContext, Vec<u8>) -> DartFnFuture<Vec<u8>> + Send + Sync>,
    pub on_sign_transaction: Arc<
        dyn Fn(u16, u16, SignatureContext, Vec<u8>, String) -> DartFnFuture<Vec<u8>> + Send + Sync,
    >,
}
